				...
				for y in dirsInOneCountry:
					if '承包方调查表' in y:
						...
						for z in CBFDCBs:
							CBF_CODE = z.split('-')[0]
							CBF_NAME = z.split('-')[1]
							CBF_CODE_NAME = CBF_CODE +'_'+ CBF_NAME
							name = z.split('-')[1]
							if name in persons:
								personFiles = os.listdir(os.path.join(resPath,name))
								for w in personFiles:
									if '归户表' in w and not os.path.exists(os.path.join(resPath,name,FBFDM+CBF_CODE+'-'+w)):
										os.rename(os.path.join(resPath,name,w),os.path.join(resPath,name,FBFDM+CBF_CODE+'-'+w))
									if '归户表' not in w and not os.path.exists(os.path.join(resPath,name,FBFDM+w)):
										os.rename(os.path.join(resPath,name,w),os.path.join(resPath,name,FBFDM+w))
